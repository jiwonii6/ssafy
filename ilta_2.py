...
# 목적구 좌표 초기화
    ...
    for target_num in target_nums:
        if balls[target_num][0] != -1 and balls[target_num][1] != -1:
            targetBall_x = balls[target_num][0]
            targetBall_y = balls[target_num][1]
            break  # 살아있는 공 찾았으면 종료

    # 1~4번 공이 다 사라졌으면 5번 공을 목표로 설정
    if targetBall_x is None and targetBall_y is None:
        if balls[5][0] != -1 and balls[5][1] != -1:
    ...
    radian = math.atan2(width,height)
